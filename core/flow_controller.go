...
type TrafficShapingController interface {
	CanPass(ctx *Context, node *ResourceNode, acquire uint32) bool
}
...
func (dc *DefaultController) CanPass(ctx *Context, node *ResourceNode, acquire uint32) bool {
...
func (dc *DefaultController) avgUsedTokens(node *ResourceNode) uint64 {
	if node == nil {
		return 0
	}
...
func (rlc *RateLimiterController) CanPass(ctx *Context, node *ResourceNode, acquire uint32) bool {
	if acquire < 0 {
		return true
	}
...
func (wpc WarmUpController) CanPass(ctx *Context, node *ResourceNode, acquire uint32) bool {
	return true
}
...
func (wpc WarmUpRateLimiterController) CanPass(ctx *Context, node *ResourceNode, acquire uint32) bool {
	return true
}
